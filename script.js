...
// ========== Character set size (only used categories) ==========
// lowercase 26, uppercase 26, numbers 10, symbols ~32
var CHARSET = { lower: 26, upper: 26, digit: 10, symbol: 32 };

function getCharsetSize(password) {
  if (!password) return { size: 0, length: 0 };
  var size = 0;
  if (/[a-z]/.test(password)) size += CHARSET.lower;
  if (/[A-Z]/.test(password)) size += CHARSET.upper;
  if (/[0-9]/.test(password)) size += CHARSET.digit;
  if (/[^a-zA-Z0-9]/.test(password)) size += CHARSET.symbol;
  return { size: size || 1, length: password.length };
}


/** True if password is a single substring repeated (e.g. abcabc, 1212) */
function hasRepeatingSubstring(password) {
  if (!password || password.length < 4) return false;
  var n = password.length;
  for (var period = 1; period <= n / 2; period++) {
    if (n % period !== 0) continue;
    var chunk = password.slice(0, period);
    var repeated = true;
    for (var i = period; i < n; i += period) {
      if (password.slice(i, i + period) !== chunk) { repeated = false; break; }
    }
...
// ========== Entropy (bits): length × log2(charset_size), only used categories ==========
// Weak patterns override: common → 0, all same → 0, repeated seq → heavy reduction
function calculateEntropy(password) {
  if (!password || password.length === 0) return 0;
  var cs = getCharsetSize(password);
  if (cs.size <= 0) return 0;

  // Base: entropy = length × log2(character_set_size)
  var baseEntropy = cs.length * Math.log2(cs.size);

  // Override: common password → instantly cracked
  if (isCommonPasswordWeak(password)) return 0;

  // Override: all characters identical → extremely weak
  if (isAllSameCharacter(password)) return 0;

  // Repeated sequences (777777, abcabc): reduce entropy significantly
  var effectiveEntropy = baseEntropy;
  if (hasRepeatingSubstring(password)) effectiveEntropy = Math.min(effectiveEntropy, 10);
  var maxRun = getMaxRepeatedRun(password);
  if (maxRun >= 3) effectiveEntropy -= Math.min(25, maxRun * 4);
  var nSeq = countNumericSequences(password) + countAlphaSequences(password);
  if (nSeq > 0) effectiveEntropy -= Math.min(20, nSeq * 4);
  if (hasKeyboardPattern(password)) effectiveEntropy -= 15;
...
// ========== Entropy tier label ==========
function getEntropyTier(entropyBits) {
  if (entropyBits < 20) return 'Instantly crackable';
...
// ========== Time to crack: combinations = charset_size^length, BigInt-safe ==========
// Attack speeds: online 10/s, offline weak (MD5) 1e10/s, offline strong (bcrypt) 1e3/s
// Uses log10 to avoid overflow; cap at trillions of years
var GUESS_RATES = {
  online: 10,
  offlineWeak: 1e10,
  offlineStrong: 1e3
...
function getTimeToCrack(password, entropyBits) {
  if (!password || password.length === 0) return '—';

  // Override: common password → instantly cracked
  if (isCommonPasswordWeak(password)) return 'Instantly crackable (common password)';

  // Override: entropy < 20 → instantly crackable
  if (entropyBits < 20) return 'Instantly crackable';

  // Time in seconds = 2^entropy / guesses_per_second
  // Work in log10 to avoid overflow: log10(time_sec) = entropy * log10(2) - log10(rate)
  var log10TimeSec = entropyBits * Math.log10(2) - Math.log10(GUESS_RATES.offlineWeak);

  // Convert to years: 1 year ≈ 3.15576e7 seconds → log10(1 year) ≈ 7.499
  var log10SecPerYear = Math.log10(31557600);
  var log10Years = log10TimeSec - log10SecPerYear;
  var years = Math.pow(10, log10Years);

  // Cap at trillions of years
  if (years >= 1e12) return 'Trillions of years (' + getEntropyTier(entropyBits) + ')';
  if (years >= 1e9) return (years / 1e9).toFixed(1) + ' billion years (' + getEntropyTier(entropyBits) + ')';
  if (years >= 1e6) return (years / 1e6).toFixed(1) + ' million years (' + getEntropyTier(entropyBits) + ')';
  if (years >= 1000) return (years / 1000).toFixed(1) + ' thousand years (' + getEntropyTier(entropyBits) + ')';
  if (years >= 1) return years.toFixed(1) + ' years (' + getEntropyTier(entropyBits) + ')';

  // Sub-year: compute from log10TimeSec
  var seconds = Math.pow(10, log10TimeSec);
  if (seconds < 1) return '&lt; 1 second (' + getEntropyTier(entropyBits) + ')';
  if (seconds < 60) return seconds.toFixed(0) + ' seconds (' + getEntropyTier(entropyBits) + ')';
  var minutes = seconds / 60;
  if (minutes < 60) return minutes.toFixed(1) + ' minutes (' + getEntropyTier(entropyBits) + ')';
  var hours = minutes / 60;
  if (hours < 24) return hours.toFixed(1) + ' hours (' + getEntropyTier(entropyBits) + ')';
  var days = hours / 24;
  return days.toFixed(0) + ' days (' + getEntropyTier(entropyBits) + ')';
}
...
function updateUI() {
  try {
    ...
    // Time to crack
    timeToCrackEl.innerHTML = password.length ? getTimeToCrack(password, entropy) : '—';
...
toggleBtn.addEventListener('click', function () {
  ...
  // Update icon: show "eye-slash" when password is visible (hiding mode)
  toggleBtn.classList.toggle('is-visible', !isCurrentlyPassword);
...
// Make sure icon starts in correct state (password is hidden → show normal eye)
